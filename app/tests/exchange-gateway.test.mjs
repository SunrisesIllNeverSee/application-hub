...
test('verified domains receive a portable hosted exchange profile',()=>{const source=read('app/api/exchange/profiles/[domain]/route.ts'); assert.match(source,/verification_status!=='verified'/); assert.match(source,/proposal_api/); assert.match(source,/request_api/); assert.match(source,/platform_fee_bps/); assert.match(source,/agreement_is_authorization:false/); assert.match(source,/authorization_is_execution:false/)})

test('human exchange manager exposes negotiation commitment authorization verification and settlement',()=>{const source=read('components/exchange/ExchangeManageClient.tsx'); for(const token of ['Contribution Commitment','Accept these exact terms','Authorize committed scope','Verify contribution','Settle exchange','Negotiation thread']) assert.match(source,new RegExp(token)); const messages=read('app/api/exchange/exchanges/[id]/messages/route.ts'); assert.match(messages,/authenticateCompany/); assert.match(messages,/authenticateProposer/); assert.match(messages,/eventType:'message'/)})
