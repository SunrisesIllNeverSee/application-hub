...
export default async function TodayPage() {
  ...
  const isPro = subData?.tier === 'pro' || subData?.tier === 'team'
  const todayCount = (unlockedToday ?? []).length
  const totalUnlocked = totalUnlockedCount ?? 0
  ...
  // Top matches not yet applied to
  const appliedIds = new Set(apps.map(a => a.program_id))
  const topMatches = (fitRows ?? []).filter(f => !appliedIds.has(f.program_id)).slice(0, 3)
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard href="/workspace" label="Active apps" value={activeApps.length}
          ...
          icon={<ListIcon />} />
        <StatCard href="/workspace" label="Closing soon" value={closingSoonApps.length}
          sub="within 14 days" accent={closingSoonApps.length > 0} warn={closingSoonApps.length > 0}
          icon={<ClockIcon />} />
      </div>
        ...
        <Section title="In progress" href="/workspace" hrefLabel="All applications">
          <div className="space-y-1">
            {activeApps.slice(0, 4).map(app => {
              ...
              return (
                <Link key={app.id} href={`/workspace/${app.program_id}`}
          ...
          <div className="flex items-center gap-4 p-4 rounded-xl bg-brand-50 dark:bg-brand-900/20 border border-brand-100 dark:border-brand-900/50">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-neutral-900 dark:text-white">
                {unanswered} question{unanswered !== 1 ? 's' : ''} waiting for your answer
              </p>
                ...
                <Link key={f.program_id} href={`/hub/${prog.slug}`}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">
                      {fitPct}% fit · composite {composite}
                    </p>
                  </div>
      ...
      {apps.length === 0 && topMatches.length === 0 && (
        <div className="card p-10 text-center">
          ...
          <h3 className="text-base font-semibold text-neutral-900 dark:text-white mb-2">Start building your arsenal</h3>
      ...
      {!isPro && (apps.length > 0 || totalUnlocked > 5) && (
        <div className="card p-4 bg-gradient-to-r from-brand-50/50 to-transparent dark:from-brand-900/10 dark:to-transparent">
          <div className="flex items-center justify-between gap-4">
            ...
            <button className="btn-primary text-xs flex-shrink-0 opacity-60 cursor-not-allowed" disabled>
              Coming soon
            </button>
          </div>
        </div>
...
function StatCard({ label, value, sub, href, accent, warn, icon }: {
  label: string; value: number; sub: string; href: string; accent?: boolean; warn?: boolean; icon: React.ReactNode
}) {
  return (
    <Link href={href} className={cn(
      'card p-4 flex flex-col gap-3 hover:shadow-card-hover transition-shadow',
      warn && 'border-warning-200 dark:border-warning-800/50'
    )}>
      <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center',
        warn ? 'bg-warning-100 dark:bg-warning-900/30 text-warning-600 dark:text-warning-400'
        : accent ? 'bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400'
