...
import { appendExchangeEvent, authenticateCompany, getExchangeAdmin } from '@/lib/exchange/server'
...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  if(!(await authenticateCompany(record.target_domain,req.headers.get('x-exchange-company-key')))) return NextResponse.json({error:'Unauthorized'},{status:401})
  ...
  const session=await stripe.checkout.sessions.create({
    mode:'payment',
    line_items:[{quantity:1,price_data:{currency,unit_amount:grossCents,product_data:{name:`Contribution exchange ${publicId}`,description:record.title}}}],
    payment_intent_data:{application_fee_amount:fees.platformFeeCents,transfer_data:{destination:payoutAccountId}},
    success_url:`${base}/exchange?settlement=success&id=${encodeURIComponent(publicId)}`,
    cancel_url:`${base}/exchange?settlement=cancelled&id=${encodeURIComponent(publicId)}`,
    metadata:{exchange_public_id:publicId,exchange_id:record.id},
  })
