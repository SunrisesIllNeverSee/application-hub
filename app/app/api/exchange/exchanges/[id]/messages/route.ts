...
function hostedStewardReply(record:{state:string;terms_hash?:string|null;escalation_required?:boolean;escalation_reasons?:string[]}){
  if(record.escalation_required) return `Message received and attached to the principal escalation${record.escalation_reasons?.length?`: ${record.escalation_reasons.join('; ')}`:''}. The exchange remains open for negotiation, but the Steward cannot cross that delegated boundary.`
  if(record.state==='negotiating') return `Message received. The current Contribution Commitment draft${record.terms_hash?` has terms hash ${record.terms_hash}`:''}. Messages do not change committed rights or authority; altered terms must be expressed in a new Commitment object and accepted by both sides.`
...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){
  ...
  const role=companyAdmin?'company_admin':domainAgent?'domain_agent':proposer?'proposer':null
  ...
  if(role==='proposer'){
    ...
    if(company?.agent_mode==='bring_your_own'){
      const delivery=await notifyDomainAgent(company,{event:'message_received',exchange_id:record.public_id,domain:record.target_domain,state:record.state,fetch:`https://mos2es.xyz/api/exchange/exchanges/${encodeURIComponent(record.public_id)}`,note:'Authenticate back to read the message. The push contains no private negotiation content.'})
    ...
    }else if(company?.agent_mode==='hosted_steward'){
