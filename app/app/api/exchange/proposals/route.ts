...
export async function POST(req:NextRequest){
  ...
  if(!parsed.success) return NextResponse.json({error:'Invalid proposal',details:parsed.error.flatten()},{status:400})
  const p=parsed.data
  if(p.honeypot) return NextResponse.json({error:'Rejected'},{status:400})
  const company=await findCompany(p.targetDomain)
  if(!company||company.verification_status!=='verified'||!company.accepts_unsolicited) return NextResponse.json({error:'Target domain is not accepting unsolicited contributions'},{status:409})
  ...
  if(error) return NextResponse.json({error:'Proposal creation failed'},{status:500})
