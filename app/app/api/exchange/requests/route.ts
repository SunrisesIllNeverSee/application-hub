...
export async function POST(req:NextRequest){
  ...
  if(!company||company.verification_status!=='verified'||!company.accepts_requests) return NextResponse.json({error:'Target domain is not accepting contribution requests'},{status:409})
  ...
  if(p.agentId){
    ...
    if(!agent||!agentKey||hashSecret(agentKey)!==agent.agent_key_hash) return NextResponse.json({error:'Invalid registered-agent credential'},{status:401})
    agentId=agent.id
  }
  ...
  const {data,error}=await admin.from('exchange_records').insert({
    ...
    offering:p.offering||null,desired_outcome:p.reason||null,evidence:p.evidenceUris,proposed_consideration:p.consideration,
    proposal_detail:proposalDetail,proposer_key_hash:hashSecret(proposerKey),referral_code:p.referralCode||null,
  }).select('id,public_id,state,target_domain,title,created_at').single()
  if(error) return NextResponse.json({error:'Request creation failed'},{status:500})
