...
async function fetchProgramQuestions(archived_question_id: string): Promise<ProgramQuestionContext[]> {
  const { data } = await supabase
...
async function fetchAnswerHistory(answer_id: string): Promise<any[]> {
  const { data } = await supabase
...
This is a read-only bridge between the launch app and deeper review workflows. It does not grade the answer itself; it gathers the answer, canonical question, program usage, program DNA, and recent answer history so external agents can produce comments, scores, or certification metadata.`,
    ...
    const answer = await fetchAnswerWithOwnership(answer_id, user_id);
    if (!answer) {
    ...
    const normalizedProgramQuestions = await fetchProgramQuestions(answer.archived_question_id);
    const programIds = Array.from(new Set(normalizedProgramQuestions.map((pq) => pq.program_id).filter(Boolean)));
    const programDna = await fetchProgramDna(programIds);
    const answerHistory = await fetchAnswerHistory(answer_id);

    const output = buildOutput(answer, archivedQuestion, normalizedProgramQuestions, programDna, answerHistory);
    ...
    return {
      content: [{ type: "text", text: formatMarkdown(output, archivedQuestion, normalizedProgramQuestions) }],
      structuredContent: output
    };
  });
}
