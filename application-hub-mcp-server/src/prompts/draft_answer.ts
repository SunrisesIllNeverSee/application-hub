...
export function registerDraftAnswer(server: McpServer) {
  server.registerPrompt(
    ...
    {
      title: "Draft Answer",
      description: "Draft a program application answer using the user's profile as context." +
        " Adapts to what this program weights.",
      argsSchema: {
...
Steps:
1. Call hub_get_question_significance with question_id="${question_id}"
   to understand the question's theme and how much it matters.
2. Call hub_get_program_dna with program_id="${program_id}" to understand what this program weights most.
3. Call hub_get_profile_answers with user_token="${user_token}" filtered to the question's theme
   — get their best existing answers.
