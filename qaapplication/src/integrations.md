...
- `06-workshop/<slug>.md` (Q + space-for-A workspace)

Plus moves the raw to `06-workshop/_shared/raw/<slug>.<ext>` and
...
don't feed back into this workspace's `07-answers/archive/`.
...
- Conflict handling: what if a capture lands while Deric is
  mid-draft in workshop? Last-write-wins is fine for raw; the
  workshop file is editorial.
...
`07-answers/archive/`), the same slug exists in the others. Flag
...
mechanically. Same check could verify `06-workshop/` entries have
...
incomplete" (e.g. unicorn-fund's empty `07-answers/archive/` because
...
- Same for `07-answers/archive/*.md` → `07-answers/index/*.md`
